...
describe('VPS Control Service', () => {
  ...
  it('invalid JSON body returns 400', async () => {
    const url = new URL('/reload/webserver', baseUrl);
  ...
  it('config validation rejects short token', async () => {
    const { createConfig: cc } = await import('../src/config.js');
    assert.throws(() => cc({ token: 'short' }), /at least 8 characters/);
  });
});
