...
- **Bearer token**: каждый запрос проверяет `Authorization: Bearer <token>` до чтения тела запроса. `/health` также требует токен.
- **Body size limit**: тело POST ограничено 64 KB, превышение возвращает 413.
...
- **sudo для systemctl**: все вызовы `systemctl` идут через `sudo` с полными путями (`/bin/systemctl`), что требует настройки sudoers (см. ниже). Сервис работает от `furniture-control`, не от root.
...
```
furniture-control ALL=(root) NOPASSWD: /bin/systemctl reload nginx, /bin/systemctl reload caddy, /bin/systemctl is-active nginx, /bin/systemctl is-active caddy, /bin/systemctl is-active furniture-vps-control
```
...
- `POST /reload/webserver` — reload через `execFile` с allowlist команд
